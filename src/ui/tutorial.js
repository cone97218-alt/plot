...
在提示词注入中应用：一旦添加了自定义属性，您就可以在模式配置的“默认行模板”中，直接使用包裹在双大括号中的英文键名作为占位符（例如：【{{title}}】奖励: {{reward}}）。系统在拼接提示词发送给 AI 时，会自动将其替换为具体的任务属性。

##### 2. 自定义徽章映射与颜色配置
UI 视觉强化：在目标主面板上，添加了自定义属性的任务会自动在标题下方渲染为醒目的小徽章（Badge），显示为 [标签名]: [内容]。
系统默认对以下常用键名做好了汉化映射和配色：
...
* durability ➜ 映射为“耐久”
* baseStats ➜ 映射为“基础属性”
* armor/weapon/accessory ➜ 映射为“防具/武器/饰品”

##### 3. 复杂属性的显示优化 (徽章智能格式化)
当自定义属性值比较复杂时，例如包含了数组、范围进度或多级嵌套的键值对，胶囊徽章将进行精美的格式化展示，而无需人工繁琐拼接：
* 范围与进度比值：如 \`{"hp":{"current":1200,"max":1200}}\` 会智能显示为 \`生命值: 1200/1200\`
* 属性与加减符号：如 \`{"armor":"丝质吊带裙","weapon":"世界树之枝"}\` 智能展示为 \`防具: 丝质吊带裙 | 武器: 世界树之枝\`
* 技能与特长数组：如 \`[{"name":"圣域结界","description":"防御"}]\` 自动抽取渲染为 \`【圣域结界】防御\` 并以 \`|\` 隔开
                        </pre>
                        ...
                        <div class="plot-display-content" style="display: flex; flex-direction: column; gap: 10px;">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 14px;">
                                <div>
                                    ...
                                    <ul style="margin: 0; padding-left: 18px; display: flex; flex-direction: column; gap: 8px;">
                                        <li>
                                            <strong>创建与扩展：</strong>在编辑具体任务时，可在底部“自定义属性”点击添加键值对。例如，您可以为战斗任务添加键名 <code>reward</code> 值为 <code>金币x100</code>。
                                        </li>
                                        <li>
                                            <strong>动态行模板渲染：</strong>在配置目标模式的“默认行模板”时，直接使用双大括号包含该键名（例如 <code>{{reward}}</code>）作为占位符。系统格式化发送给 AI 时，会自动将其解析替换为实际属性值。
                                        </li>
                                    </ul>
                                </div>
                                ...
                                <div>
                                    ...
                                    <ul style="margin: 0; padding-left: 18px; display: flex; flex-direction: column; gap: 8px;">
                                        ...
                                        <li>
                                            <strong>系统内置别名：</strong>系统对常用属性（如 <code>reward</code> 奖励、<code>clue</code> 线索、<code>durability</code> 耐久、<code>baseStats</code> 基础属性等）内置了汉化标签名。
                                        </li>
                                        <li>
                                            <strong>自定义标签与色彩：</strong>在目标齿轮的“显示 (Display)”标签页下，找到“自定义徽章映射与颜色”。添加映射规则后，可为任意属性键值绑定汉化微标文本并定制边框色彩。
                                        </li>
                                    </ul>
                                </div>

                                <div>
                                    <div style="font-weight: bold; color: var(--SmartThemeEmColor); font-size: 0.95em; border-left: 3px solid var(--SmartThemeEmColor); padding-left: 6px; margin-bottom: 6px;">3. 复杂属性的智能显示优化 (徽章胶囊)</div>
                                    <p style="margin: 0; opacity: 0.8;">前端看板胶囊会自动以高可读性格式呈现复杂数据：</p>
                                    <ul style="margin: 4px 0 0 0; padding-left: 18px; display: flex; flex-direction: column; gap: 4px; opacity: 0.85; font-size: 0.95em;">
                                        <li><strong>范围与进度：</strong><code>{"hp":{"current":1200,"max":1200}}</code> ➜ <code>生命值: 1200/1200</code></li>
                                        <li><strong>对象与装备属性：</strong><code>{"armor":"丝质吊带裙","weapon":"世界树之枝"}</code> ➜ <code>防具: 丝质吊带裙 | 武器: 世界树之枝</code></li>
                                        <li><strong>数组对象名录：</strong><code>[{"name":"圣域结界","desc":"防御"}]</code> ➜ <code>【圣域结界】防御</code></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
...
##### 1. 主聊天注入大模板
* {{plot_goals_active}} : 进行中任务
* {{plot_goals_pinned}} : 置顶任务
* {{plot_goals_complete}} : 已完成任务
* {{plot_goals_failed}} : 已失败任务
* {{plot_goals_hidden}} : 已隐藏任务
* {{plot_goals_all}} : 全部任务

##### 2. 默认行模板占位符
* {{title}}, {{desc}}, {{status}}, {{type}}
* 自定义属性字段: {{key}}
                        </pre>

                        <div class="plot-display-content" style="display: flex; flex-direction: column; gap: 10px;">
                            <p style="margin: 0 0 8px 0; opacity: 0.9; font-size: 0.9em; border-bottom: 1px dashed var(--SmartThemeBorderColor); padding-bottom: 8px;">精细控制任务列表在大模型面前的展示形态：</p>
...
* 这样，若任务中添加了 difficulty: 极难 属性，主面板上就会完美呈现出一个红色的“挑战难度: 极难”徽章，使任务界面和状态一目了然！
                        ...
                        <div class="plot-display-content" style="display: flex; flex-direction: column; gap: 10px;">
                            ...
                            <d##### 2. 动态分类状态宏占位符 (Dynamic Category & Status Macros)
系统根据您任务卡片上实际填写的“分类（category）”名称，动态且全自动地为每一种状态注册对应的解算占位符。它同时支持单数与复数形式（自动加 's' 后缀）。

例如，若您的任务中存在 "quest"、"item"、"skill" 等分类，以下占位符即自动生效：
- {{plot_quests_active}} / {{plot_quest_active}} : 进行中的任务
- {{plot_items_active}} / {{plot_item_active}} : 已配备/装备的物品
- {{plot_items_complete}} / {{plot_item_complete}} : 背包中的物品
- {{plot_skills_active}} / {{plot_skill_active}} : 已装配的技能
- {{plot_skills_complete}} / {{plot_skill_complete}} : 已习得的技能/天赋
- {{plot_characters_active}} / {{plot_character_active}} : 队伍中同行的伙伴
- {{plot_achievements_complete}} / {{plot_achievement_complete}} : 已达成的成就

※ **使用生效场景**：
1. **SillyTavern 任何文本输入框**：如角色设定、首条消息、系统提示词、世界书词条等。系统会自动拦截并替换为最新内容。
2. **本插件提示词工作台**：在编写各类 Prompts 模板时直接引用。
3. **全局注入大模板 / 任务模板**：通过子层级宏进行级联拼接嵌套。

※ **自定义分类无缝支持**：若您创建了自定义分类（如 "bloodline" 血统），系统会实时自动生成 {{plot_bloodlines_active}} 或 {{plot_bloodline_active}} 等宏。直接在全局注入模板中使用即可，完全不需要修改代码！
                        </pre>
                        ...
                        <div class="plot-display-content" style="display: flex; flex-direction: column; gap: 10px;">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 14px;">
                                ...
                                <div>
                                    <div style="font-weight: bold; color: var(--SmartThemeEmColor); font-size: 0.95em; border-left: 3px solid var(--SmartThemeEmColor); padding-left: 6px; margin-bottom: 6px;">2. 动态分类宏命名规范与使用范围</div>
                                    <p style="margin: 0; opacity: 0.8;">宏的格式统一为 <code>{&#123;plot_[类别复数或单数]_[状态英文]&#125;}</code>。这些宏已被注入到酒馆底层。支持自动单复数，可用于角色卡设定、系统提示词、世界书、及插件提示词卡片模板中。</p>
                                </div>

                                <div>
                                    <div style="font-weight: bold; color: var(--SmartThemeEmColor); font-size: 0.95em; border-left: 3px solid var(--SmartThemeEmColor); padding-left: 6px; margin-bottom: 6px;">3. 复杂属性的智能显示优化 (胶囊徽章)</div>
                                    <p style="margin: 0; opacity: 0.8;">任务自定义属性中如果包含复杂的数组或 JSON 对象，前端看板胶囊会自动以高可读性格式呈现：</p>
                                    <ul style="margin: 4px 0 0 0; padding-left: 18px; display: flex; flex-direction: column; gap: 4px; opacity: 0.85; font-size: 0.95em;">
                                        <li><strong>范围与进度：</strong>如 <code>{"hp":{"current":1200,"max":1200}}</code> 智能渲染为 <code>生命值: 1200/1200</code>。</li>
                                        <li><strong>对象与装备属性：</strong>如 <code>{"armor":"丝质吊带裙","weapon":"世界树之枝"}</code> 智能渲染为 <code>防具: 丝质吊带裙 | 武器: 世界树之枝</code>。数值属性自带加减号显示。</li>
                                        <li><strong>数组对象名录：</strong>如 <code>[{"name":"圣域结界","desc":"防御护罩"}]</code> 智能渲染为 <code>【圣域结界】防御护罩</code> 并以 <code>|</code> 漂亮隔开。</li>
                                    </ul>
                                </div>属性键值（如 <code>difficulty</code>）绑定汉化微标文本（如 <code>难度</code>）并定制特定的霓虹边框颜色。面板会立即刷新呈现出高颜值的任务状态！
                                        </li>
                                    </ul>
                                </div>
                            </div>
